...
%DIF DEL 1st_version/agujournaltemplate.tex   Wed Dec 23 17:06:24 2020
%DIF ADD 2nd_version/agujournaltemplate.tex   Tue Dec 29 12:21:34 2020
...
\citep{Kretschmer2018}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Thompson2002,Lehtonen,Tomassini2012,Kretschmer2018}}\hspace{0pt}%DIFAUXCMD
. SSWs also play a key role in seasonal to sub-seasonal forecasts \mbox{%DIFAUXCMD
\citep{Domeison2019-1, Domeison2019-2}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend In reanalysis datasets, SSWs occur at an average rate of 0.6 events/winter but this varies markedly over the record \citep{Butler2015} suggesting the possibility of variability on much longer timescales. For example, observational studies have noted a hiatus in the 1990s when very few major SSW events occurred \citep{Butler2015,Pawson1999,Shindell1999} while, in contrast, the early 21st century displayed a remarkable number of consecutive winters containing SSW events \citep{Manney2005}\DIFdelbegin \DIFdel{as well as an anomalously warm stratospheric NH winter polar cap}\DIFdelend . 

Despite a significant body of work aimed at understanding the nature of SSWs and their impacts on mid-latitude surface climate, variability of their occurrence on decadal to multi-decadal timescales is not well understood. Multi-decadal stratospheric variability has been considered in the context of forced anthropogenic warming signals in GCMs. For example \cite{Garfinkel2017} analyse decadal-scale variations in polar vortex strength in a set of historical simulations and propose that an observed hiatus in Eurasian surface warming was most likely due to variability in midwinter vortex strength. \DIFaddbegin \DIFadd{Similarly, \mbox{%DIFAUXCMD
\cite{cohen2009} }\hspace{0pt}%DIFAUXCMD
find decadal scale variations in planetary wave forcing of the vortex in a suite of CMIP3 models as well as NCEP–NCAR renalysis. They suggest these fluctuations leads to a modulation of the global warming signal in late boreal winter surface temperatures. }\DIFaddend Whether the vortex variability was forced by greenhouse gas concentrations or arose through internal variability \DIFdelbegin \DIFdel{was not established. \mbox{%DIFAUXCMD
\cite{Schimanke2011} }\hspace{0pt}%DIFAUXCMD
find similar }\DIFdelend \DIFaddbegin \DIFadd{in these studies was not fully established but \mbox{%DIFAUXCMD
\cite{Garfinkel2015} }\hspace{0pt}%DIFAUXCMD
used a subset of the simulations analysed in \mbox{%DIFAUXCMD
\cite{Garfinkel2017} }\hspace{0pt}%DIFAUXCMD
and linked a decadal trend (1980-2009) in late winter vortex strength to SST variability. On the other hand, \mbox{%DIFAUXCMD
\cite{Seviour2017} }\hspace{0pt}%DIFAUXCMD
analysed observed variations between 1980 and 2016 and concluded that the vortex variability was primarily internally generated. \mbox{%DIFAUXCMD
\cite{Schimanke2011} }\hspace{0pt}%DIFAUXCMD
noted  }\DIFaddend multidecadal scale variations \DIFdelbegin \DIFdel{of the polar vortex (specifically SSWs) }\DIFdelend \DIFaddbegin \DIFadd{in SSW occurence }\DIFaddend with periods of approximately 52 years in a multi-century GCM integration and \DIFdelbegin \DIFdel{propose }\DIFdelend \DIFaddbegin \DIFadd{demonstrated }\DIFaddend coherent variability in other parts of the climate system, including vertically propagating planetary wave activity, Eurasian snow cover and Atlantic sea surface temperatures (SSTs). \DIFdelbegin \DIFdel{Despite }\DIFdelend \DIFaddbegin \DIFadd{However, despite }\DIFaddend providing some indications of externally driven variability, results from this study are not conclusive, since the GCM used (EGMAM: ECHO‐G with Middle Atmosphere Model) exhibits significant bias in mean SSW rate compared to reanalyses (2 events per decade)\DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{. This }\DIFaddend means that their findings may not be fully representative of the observed stratosphere \DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{and the }\DIFaddend authors note that further simulations are required to understand this variability. \cite{Manzini2012} explore causes of 20 year period variability in a simulation with prescribed, pre-industrial SSTs. They propose that, given the boundary conditions in the simulations are fixed, such variability must be internally generated. \cite{Butchart2000} suggest that decadal variability in vortex strength as well as SSW frequency may originate from feedbacks caused by the non linear nature of boreal winter stratospheric dynamics. Both works show that these internally induced signals significantly influence mid-latitude surface variability, forcing similar period signals in the NAO and \DIFdelbegin \DIFdel{north }\DIFdelend \DIFaddbegin \DIFadd{North }\DIFaddend Atlantic SSTs.

A region often considered in studies of vortex strength variability is the equatorial stratosphere. The primary mechanism for coupling between these regions is between the Quasi Biennial Oscillation (QBO) and the vortex. An association between the phase of the QBO and the strength of the polar vortex was first proposed by \cite{HoltonJamesRTan1980} and \cite{Holton1982} who found that the polar vortex exhibited a strengthening when the QBO near the 50\,hPa level was in its westerly phase (QBO-W) compared to its easterly phase (QBO-E). This link, usually referred to as the Holton-Tan (HT) \DIFdelbegin \DIFdel{link}\DIFdelend \DIFaddbegin \DIFadd{effect}\DIFaddend , has been reported in subsequent studies with more comprehensive observations as well as in modelling studies using GCMs including the Met Office Hadley Centre Model 2 (HadGEM2) \DIFdelbegin \DIFdel{, the predecessor }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Watson2014} }\hspace{0pt}%DIFAUXCMD
and other Met Office models \mbox{%DIFAUXCMD
\citep{Garfinkel2018} }\hspace{0pt}%DIFAUXCMD
based on predecessors }\DIFaddend of the model considered in this study\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Baldwin1991,Pascoe2005,Lu2008,Watson2014}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{. Separate modelling studies also report a HT effect \mbox{%DIFAUXCMD
\citep{Baldwin1991,Pascoe2005,Lu2008}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend A number of physical mechanisms have been proposed to account for the observed coupling between the QBO and the \DIFaddbegin \DIFadd{stratospheric polar }\DIFaddend vortex that involve a QBO influence on wave propagation into the winter stratosphere \citep{Baldwin2001}.  
...
Vortex variability has also been closely associated with variations in winter surface climate that can determine the strength of mid-latitude tropospheric wave driving. Among the most notable of these is the climatological low pressure system over the Aleutian Islands in the Bering sea - the Aleutian low (AL). The \DIFdelbegin \DIFdel{depth }\DIFdelend \DIFaddbegin \DIFadd{intensity }\DIFaddend of the AL has been shown to modulate vertical planetary wave propagation into the vortex region \DIFdelbegin \DIFdel{via coupling with the corresponding stratospheric high on the vortex edge \mbox{%DIFAUXCMD
\citep{Woo2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Woo2015, Garfinkel2010, Manzini2006}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The effect has been found in reanalysis \citep{Hu2018}  as well as modelling studies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{kren2016, Kang2017}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{kren2016, Kang2017,Taguchi2006}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The AL is a key indicator of Pacific climate variability with teleconnections \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend both tropical and mid-latitude climate \citep{Nitta1989, Trenberth1994, Zhang1997} and it varies significantly on decadal to multi-decadal timescales. \cite{Overland1999} note that 10 year mean values of SLP over the AL region exhibit fluctuations of up to 35\% of the climatological mean. Subsequent studies corroborate the presence of these decadal scale fluctuations: \cite{SUGIMOTO2009} and \cite{Minobe} show 20 year fluctuations in intensity and centre of action of the AL while \cite{Raible2005} propose a 50-60 year trend in AL \DIFdelbegin \DIFdel{depth}\DIFdelend \DIFaddbegin \DIFadd{intensity}\DIFaddend , suggesting the existence of even longer timescale variability. 

Further surface features linked to vortex variability involve tropical SSTs. For example the SST anomalies over the Eastern Pacific region associated with the El Ni\~{n}o Southern Oscillation (ENSO) have been shown to induce a stratospheric vortex circulation response via a pathway \DIFdelbegin \DIFdel{including }\DIFdelend \DIFaddbegin \DIFadd{involving }\DIFaddend the AL \citep{Domeison2019}. A positive ENSO phase is associated with a deepening of the AL which promotes stronger planetary wave forcing of the middle atmosphere. This teleconnection has been found extensively in observation based studies \citep{Garfinkel2008, Ineson2009, Smith2012}  as well as modelling studies \citep{Bell2009, Domeison2015, Manzini2006, Richter2015}. However, the connection's robustness has also been shown to vary between ENSO events \citep{deser2017, Iza2016} \DIFdelbegin \DIFdel{. Other Tropical }\DIFdelend \DIFaddbegin \DIFadd{and between decades \mbox{%DIFAUXCMD
\citep{ayarzaguena2018} }\hspace{0pt}%DIFAUXCMD
suggesting elements of non-stationarity in the teleconnection. SSTs in other tropical }\DIFaddend regions also exhibit \DIFdelbegin \DIFdel{elements of connection to vortexstrength variability: }\DIFdelend \DIFaddbegin \DIFadd{coherence with the vortex. }\DIFaddend \cite{Rao2017} show that Tropical Atlantic SSTs \DIFdelbegin \DIFdel{appear to }\DIFdelend give rise to a vortex response although \DIFdelbegin \DIFdel{the response }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend is highly variable throughout the season while \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Rao2015} }\hspace{0pt}%DIFAUXCMD
also propose a Tropical }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Fletcher}}\hspace{0pt}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{Fletcher_model} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{Rao2015} }\hspace{0pt}%DIFAUXCMD
propose a tropical }\DIFaddend Indian Ocean (TIO) connection\DIFdelbegin \DIFdel{which manifests as a competing mechanism with the ENSO-vortex effect, i. e. positive }\DIFdelend \DIFaddbegin \DIFadd{. Positive }\DIFaddend TIO SST anomalies lead to a reduced strength of the AL that weakens the Rossby wave forcing of the \DIFdelbegin \DIFdel{middle atmosphere}\DIFdelend \DIFaddbegin \DIFadd{vortex, an opposite effect to the ENSO-vortex connection where positive SST anomalies leads to vortex weakening}\DIFaddend .

\DIFaddbegin \DIFadd{Other surface forcings have been shown to modulate vortex variability via alterations of tropospheric stationary wave activity. For example, Eurasian snow coverage in October has been shown to exhibit a connection with the mid winter vortex in observational and model data \mbox{%DIFAUXCMD
\citep{Garfinkel2010, Cohen2007}}\hspace{0pt}%DIFAUXCMD
, although \mbox{%DIFAUXCMD
\cite{Henderson2018} }\hspace{0pt}%DIFAUXCMD
highlight that many of the underlying processes behind this connection are not well understood. Sea ice extent over the Kara and Barent regions has also been proposed as an influence on planetary wave forcing of the vortex via alterations in ocean-atmosphere heat flux \mbox{%DIFAUXCMD
\citep{Kim2020, Nakamura2016}}\hspace{0pt}%DIFAUXCMD
. The resulting influence on SSW occurrence forms the basis of a proposed pathway whereby increased sea ice reduction leads to a negative Arctic Oscillation (AO) and severe NH winter weather. Finally, \mbox{%DIFAUXCMD
\cite{Hirota2018} }\hspace{0pt}%DIFAUXCMD
have proposed that Arctic sea ice fraction variations may modulate the strength of the Holton-Tan relationship  and sea ice loss has also been linked to the 2016 QBO disruption event \mbox{%DIFAUXCMD
\cite{labe2019}}\hspace{0pt}%DIFAUXCMD
. 
}

\DIFaddend Although substantial effort has been applied to characterising SSWs and their underlying mechanisms, there is little understanding of periods of hiatus (such as that in the 1990s) and consecutive-event years (early-mid 2000s), primarily because of the short record of reliable observations as well as the complexities \DIFaddbegin \DIFadd{and often non-stationary nature }\DIFaddend of the multiple observed teleconnections. Much longer time-series are required to successfully identify and understand the source of decadal and multi-decadal scale variability. In the meantime, analysis of variability and teleconnections in long climate model simulations may help to understand these processes. 

In this work, we analyse long-term variability of the stratospheric polar vortex in a 1000-yr pre-industrial (piControl) simulation of the UK Earth System Model (UKESM). The absence of external forcings such as greenhouse gas increases, volcanic and solar variations allows us to examine sources of long-term variability that are internally generated within the climate system. We identify intervals containing high and low SSW rates and analyse their variability, with a focus on multi-decadal scales. Improved  understanding and representation of stratospheric variability will help to improve predictions of NH winter surface weather and climate \citep{Kidston2015,gray2020}. A variety of techniques \DIFdelbegin \DIFdel{including wavelet spectral decomposition and cross-spectrum analysis }\DIFdelend are employed to examine associations with those parts of the climate system that are known to exhibit long-term memory, namely the tropical SSTs and the related Aleutian Low. In \DIFdelbegin \DIFdel{addition, we }\DIFdelend \DIFaddbegin \DIFadd{particular, a wavelet spectral decomposition and cross-spectrum analysis is employed to overcome some of the difficulties with non stationary signals that may arise. We also }\DIFaddend investigate interactions between the polar vortex and the QBO as a potential source of internally-driven variability. The latter reveals an unexpected source of multi-decadal scale variability associated with the \DIFdelbegin \DIFdel{strength }\DIFdelend \DIFaddbegin \DIFadd{amplitude }\DIFaddend and vertical depth of the QBO. The paper is structured as follows: Section 2 sets out the GCM used in the investigation, the spectral analysis method (wavelet analysis) and relevant climate indices. Section 3 presents results from the analysis. Section 4 discusses findings and concludes.

\section{\DIFdelbegin \DIFdel{Model and Data}\DIFdelend \DIFaddbegin \DIFadd{Methodology}\DIFaddend }
\subsection{Model Configuration}
The first version of the UK Earth System Model (henceforth referred to as UKESM) is the most recent configuration of the MetOffice unified model (the UM) \citep{Mulcahy2018}. UKESM is a stratosphere resolving coupled ocean-atmosphere-land-sea ice model. The Atmospheric component is GA7.1 \DIFdelbegin \DIFdel{which contains }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend 85 vertical \DIFdelbegin \DIFdel{atmospheric levels with a maximum altitude of }\DIFdelend \DIFaddbegin \DIFadd{levels from the surface to }\DIFaddend 85km\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend 35 \DIFdelbegin \DIFdel{levels above a height of }\DIFdelend \DIFaddbegin \DIFadd{of which are above }\DIFaddend 18km \citep{Walters2019, Williams2018}. \DIFdelbegin \DIFdel{For this study GA7.1 }\DIFdelend \DIFaddbegin \DIFadd{The model }\DIFaddend is run at N96 horizontal resolution (approximately 135 km near the equator). The ocean model used is GO6.0 \citep{Storkey2018} which contains 75 levels and runs at 1${^\circ}$ horizontal resolution. Land surface and sea-ice processes are represented by JULES \citep[GL7.0,][]{Walters2019} and CICE  \cite[GSI8.1,][]{Ridley2018} models respectively, while ocean biochemistry is added through MEDUSA \citep{Yool2013}. UKESM also includes a fully interactive chemistry scheme via coupling with the UK Chemistry and Aerosols model \citep[UKCA,][]{Mulcahy2018}. We utilise a 1000 year pre-industrial (PI) control simulation of UKESM submitted to CMIP6 which is spun-up to achieve initial model equilibrium following the method outlined in \cite{Yool20}. This run is forced using CMIP6 pre-industrial values for concentrations of major GHGs (global mean 284.317ppm $CO_2$, 808.25ppb $CH_4$, 273.02ppb $N_2O$). While there are no volcanic eruptions in the simulation, background stratospheric volcanic aerosols are set to climatological values between 1850 and 2014 estimated from satellite products and other model simulations \citep{Menary2018}. We choose a PI control for this analysis to examine internal variability in SSWs on multi-decadal timescales. \DIFdelbegin \DIFdel{For comparison between the model and the real atmosphere we utilise }\DIFdelend \DIFaddbegin \DIFadd{To verify that the model reproduces relevant features of the climate system we compare with }\DIFaddend the ERA-Interim \DIFdelbegin \DIFdel{dataset described fully in }\DIFdelend \DIFaddbegin \DIFadd{reanalysis dataset }\DIFaddend \cite{Dee2011}. 

\DIFaddbegin \subsection{\DIFadd{Linear Regression Analysis}}
\DIFadd{We employ a multi-linear regression technique to give an estimate of the relative contributions to SSW variability from the QBO, ENSO and the AL following the method outlined in \mbox{%DIFAUXCMD
\cite{Krzywinski}}\hspace{0pt}%DIFAUXCMD
. We model an SSW timeseries of length $n$ which we denote $y$ as 
}

\begin{equation} \DIFadd{\label{regression}
\hat{y} = \beta_0 + \beta_{1}QBO + \beta_{2}ENSO + \beta_{3}AL
}\end{equation}

\DIFadd{where $\beta_j$ denotes the coefficient of the corresponding index and $\hat{y}$ is the prediction of $y$. We calculate the best estimate for each $\beta$ using an ordinary least square (OLS) estimator which minimises the sum of squared error (SSE) between the predicted $\hat{y}$ and the real time series $y$ with respect to each coefficient. the SSE is given by $SSE = \sum_i^n{(\hat{y}_i - y_i)^2}$.
}

\DIFadd{We can compare the estimated magnitude of the coefficient for each index to analyse the respective contributions to SSW variability. We can also calculate standard error ranges for $\beta$ estimates. The standard error on an estimated value of a true $\beta_j$ (denoted by $\hat{\beta_j}$) is given by 
}
...
s_j = s_0 2\DIFdelbegin \DIFdel{^{j \deltaj}}\DIFdelend \DIFaddbegin \DIFadd{^{j \delta j}}\DIFaddend ,\DIFaddbegin \\ \DIFaddend j = 0, 1, ..., J
\end{equation}
...
We utilise the definition of an SSW event from \cite{Butler2015}. An event is recorded when the ZMZW at 60$^\circ$N on the 10\,hPa level transitions from westerly to easterly during NH winter months (November - March). The day on which this reversal occurs is referred to as the central date. After this date, the ZMZW must recover to westerly for a period of \DIFaddbegin \DIFadd{at least }\DIFaddend 10 consecutive days (which is the approximate radiative timescale of the mid-stratosphere\DIFdelbegin \DIFdel{thus allowing the vortex to recover}\DIFdelend ) before another event can be recorded.  If, after the central date, the ZMZW does not recover to westerly for \DIFaddbegin \DIFadd{at least }\DIFaddend 20 consecutive days before the end of April, the warming is classified as a final warming\DIFdelbegin \DIFdel{, when the westerly flow permanently breaks down for the season}\DIFdelend .

We analyse variability in tropical SSTs in four regions identified by \cite{Scaife2016} as key to affecting Rossby wave propagation and interactions with stratospheric winds. The regions are defined as the Tropical Atlantic ($5^{\circ}S$-$5^{\circ}N, 60^{\circ}W$–$0^{\circ}W$), Tropical East Pacific ($5^{\circ}S$–$10^{\circ}N, 160^{\circ}$–$270^{\circ}E$), Tropical West Pacific ($5^{\circ}S$–$25^{\circ}N, 110^{\circ}$–$140^{\circ}E$) and Tropical Indian Ocean ($5^{\circ}S$–$10^{\circ}N, 45^{\circ}$–$100^{\circ}E$). Additionally we calculate an El Ni\~{n}o Southern Oscillation (ENSO3.4) index \DIFaddbegin \DIFadd{as the SST anomaly in the region $5^{\circ}S$-$5^{\circ}N, 170^{\circ}W$–$120^{\circ}W$ }\DIFaddend following \cite{Trenberth2001}. We use an index to track the \DIFdelbegin \DIFdel{depth }\DIFdelend \DIFaddbegin \DIFadd{intensity }\DIFaddend of the Aleutian low pressure system \DIFaddbegin \DIFadd{based on the method of \mbox{%DIFAUXCMD
\cite{Chen2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend as the projection of the first Principal Component of \DIFdelbegin \DIFdel{early winter (Sep-Nov) }\DIFdelend \DIFaddbegin \DIFadd{winter }\DIFaddend mean sea level pressure (MSLP) anomalies averaged over the region $120^{\circ}$–$240^{\circ}E, 20^{\circ}$–$70^{\circ}N$. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Additionally we define the QBO as the }\DIFdelend \DIFaddbegin \DIFadd{We employ an EOF based method as opposed to a fixed box average to allow for the fact that the centre of the model AL may not line up well with observations. The month range used for studies into AL-vortex teleconnections varies somewhat with \mbox{%DIFAUXCMD
\cite{Overland1999} }\hspace{0pt}%DIFAUXCMD
using both Jan-Feb and Nov-Mar while \mbox{%DIFAUXCMD
\cite{Hu2018} }\hspace{0pt}%DIFAUXCMD
use a core winter metric (Dec-Feb). Unless stated otherwise we use the same month range as our SSW definition (Dec-Mar); for all analyses, tests were performed to check that the results were not unduly sensitive to the choice. An index for the Pacific Decadal Oscillation (PDO) was determined following the methodology of \mbox{%DIFAUXCMD
\cite{Mantua_1997} }\hspace{0pt}%DIFAUXCMD
using the leading Principal Component of Pacific basin ($120^{\circ}$–$240^{\circ}E$) SST anomalies poleward of 20$^{\circ}$N. Finally, a QBO index was defined by a variety of measures (see section 3 for further discussion), using the }\DIFaddend monthly mean ZMZW averaged between $\pm5^{\circ}$ latitudes at various stratospheric pressure levels (15\,hPa, 20\,hPa, 30\,hPa, 50\,hPa, 70\,hPa) as well as two \DIFdelbegin \DIFdel{integrated metrics as the vertical mean winds between the 15}\DIFdelend \DIFaddbegin \DIFadd{'deep QBO' indices computed  by taking the average of the ZMZW between  15-30}\DIFaddend \,hPa \DIFdelbegin \DIFdel{and 30\,hPa levels (defined }\DIFdelend \DIFaddbegin \DIFadd{(as }\DIFaddend in \cite{Andrews2019}) and between \DIFdelbegin \DIFdel{20}\DIFdelend \DIFaddbegin \DIFadd{20-50}\DIFaddend \,hPa \DIFdelbegin \DIFdel{and 50\,hPa}\DIFdelend \DIFaddbegin \DIFadd{to identify QBO phases that exhibit winds of the same sign over a relatively large vertical extent}\DIFaddend . 

\section{Results}

\subsection{Modes of Stratospheric Variability}
We begin by analysing the representation of modes of stratospheric variability in the UKESM piControl simulation. As described in section 1, the winter polar stratospheric vortex  exhibits substantial variability. In some years the westerly winds of the vortex are relatively strong and undisturbed while in other years the vortex is weakened by wave disturbances that in extreme cases can lead to SSWs. The average SSW rate over the full 1000 years of the UKESM simulation is 0.54 events/winter. This represents a marginal underestimation compared to ERA-Interim (0.62 events/winter between 1979 and 2019) but is within 1 standard error of the observations. The model adequately represents the seasonal distribution of SSWs compared to the reanalysis dataset, as shown in figure 1, \DIFdelbegin \DIFdel{with most SSW events occurring in February and March. }\DIFdelend \DIFaddbegin \DIFadd{but exhibits too many warming events in November (not shown) and an underestimation of Jan and Feb warming rates (see \mbox{%DIFAUXCMD
\cite{Andrews2020} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{Menary2018} }\hspace{0pt}%DIFAUXCMD
for further details). This 'cold pole' bias is well known and relatively common in GCMs. On the other hand, we note that validation of this pre-industrial control simulation with ERA-Interim data is not optimum. The sample sizes of the ERA-interim data and the model are very different and could give rise to differences in distributions \mbox{%DIFAUXCMD
\citep{Horan2017} }\hspace{0pt}%DIFAUXCMD
and the ERA-Interim SSW rates may be influenced by anthropogenic forcing, the impact of which is not well understood \mbox{%DIFAUXCMD
\citep{Ayarz2020}}\hspace{0pt}%DIFAUXCMD
. In all analyses presented in the following sections, tests have been performed to ensure that the results are not sensitive to the inclusion or exclusion of November SSW rates. 
}\DIFaddend 
...
The model exhibits variability in SSW frequency comparable to observations, including \DIFdelbegin \DIFdel{hiatus }\DIFdelend \DIFaddbegin \DIFadd{both hiatus and consecutive SSW }\DIFaddend intervals. Figure 2 shows a sample 40-yr interval of the polar vortex zonal wind strength from the UKESM simulation compared with a similar length from the ERA Interim reanalyses. An extended interval of mainly westerly anomalies indicating a strengthened vortex and lack of SSWs can be seen towards the end of the 40-yr interval, similar to the 1990s in ERA-Interim when only 2 SSW events were recorded in the decade. \DIFdelbegin \DIFdel{UKESM }\DIFdelend \DIFaddbegin \DIFadd{The simulation }\DIFaddend contains 8 such \DIFaddbegin \DIFadd{hiatus }\DIFaddend intervals with at least 10 consecutive years with no SSWs, the longest of which lasts 16 years. \DIFdelbegin \DIFdel{The model time series }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, the simulation }\DIFaddend only contains 2 intervals in which 10 consecutive years exhibit at least 1 SSW\DIFdelbegin \DIFdel{, although when }\DIFdelend \DIFaddbegin \DIFadd{. However, if }\DIFaddend the threshold interval width \DIFdelbegin \DIFdel{is shortened }\DIFdelend \DIFaddbegin \DIFadd{for identifying hiatus and consecutive-SSW intervals is shortened from 10 }\DIFaddend to 5 years\DIFaddbegin \DIFadd{,  then }\DIFaddend 9 \DIFdelbegin \DIFdel{consecutive-event }\DIFdelend \DIFaddbegin \DIFadd{consecutive-SSW }\DIFaddend intervals and 25 hiatus intervals are found. These statistics indicate that UKESM is not only able to reproduce the mean state characteristics of SSW events but also decadal-scale variations in SSW rate, underlining its  suitability for this study. 
...
There is evidence of coupling between the two major modes of stratospheric variability in the model, giving rise to a Holton-Tan relationship \citep{Anstey20}. Figure 4 shows height-latitude cross-sections of NH winter zonal wind differences between \DIFdelbegin \DIFdel{composites defined by the QBO at various }\DIFdelend \DIFaddbegin \DIFadd{QBO E-W composites defined at various equatorial }\DIFaddend levels. The familiar pancake structure of alternating easterly / westerly differences is present at equatorial latitudes, indicative of the QBO phase but there is also a response at high latitudes. In good agreement with observations the largest high latitude response amplitude is seen when  the QBO is defined at 50\,hPa, with anomalously weaker polar vortex strength in QBO-E than in QBO-W years. Higher levels (15\,hPa and 20\,hPa) show little significant QBO-vortex coupling. For comparison we also show in figure 4 the composite different response for QBO composites selected on the basis of the average QBO winds over a greater depth of the equatorial atmosphere (15-30 hPa and 20-50 hPa)\DIFdelbegin \DIFdel{so the impact of vertical coherence in the QBO shear zones can be assessed}\DIFdelend \DIFaddbegin \DIFadd{. We note that while this QBO definition will select some of the same years as in the separate single-level composite definitions, it is specifically designed to identify only QBO  phases that have extended vertical coherence}\DIFaddend , following \citep{Gray2018} and \cite{Andrews2019}\DIFdelbegin \DIFdel{. Not surprisingly, the high latitude responses to the deep-QBO selections resemble the }\DIFdelend \DIFaddbegin \DIFadd{, so the resulting composite  differences in figure 4 will not necessarily be an }\DIFaddend average of the \DIFdelbegin \DIFdel{individual responses but interestingly}\DIFdelend \DIFaddbegin \DIFadd{corresponding single-level differences.   Interestingly}\DIFaddend , the 15--30 hPa deep-QBO selects years that exhibit \DIFdelbegin \DIFdel{coincidence of both }\DIFdelend \DIFaddbegin \DIFadd{not only }\DIFaddend a weaker polar vortex \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{in QBO-E but also }\DIFaddend a weaker sub-tropical tropospheric jet (see 200 hPa, 30--40N). This results in a more coherent response in the mid-latitude troposphere and at the surface, in excellent agreement with the results of \cite{Gray2018} and \cite{Andrews2019}. 
...
\begin{tabular}{|p{3cm}||p{3cm}|p{3cm}|}
 ...
 \hline
 \DIFaddFL{Regression Variable}& \DIFaddFL{Coefficient}& \DIFaddFL{p value}\\
 \hline
 \DIFaddFL{ENSO3.4  }& \DIFaddFL{0.1625$^+_-$0.035}& \DIFaddFL{0.0002}\\
 \DIFaddFL{AL  }&   \DIFaddFL{-0.0927$^+_-$0.04  }& \DIFaddFL{0.048}\\
 \DIFaddFL{deep QBO  }& \DIFaddFL{-0.1993$^+_-$0.03}&\DIFaddFL{0.0001}\\
 \hline
 \end{tabular}
\begin{center}
\caption{\DIFaddFL{Summary of results from regression analysis of SSWs per NH winter time series.}}
\end{center}
\end{table}
...
A more comprehensive assessment of the long-term variability of SSWs can be made using a wavelet power spectrum approach. We count the number of SSWs in each winter season (Dec-Mar) and calculate the corresponding wavelet power spectrum\DIFdelbegin \DIFdel{(figure 6) }\DIFdelend \DIFaddbegin \DIFadd{, shown in figure 6. As described above, the analysis highlights the presence of power in the signal as a function of frequency (period in years, along the y-axis) and as a function of time (year of simulation along the x-axis)}\DIFaddend . As expected, there is \DIFdelbegin \DIFdel{a QBO-like signal of }\DIFdelend \DIFaddbegin \DIFadd{an intermittent but relatively persistent signal with }\DIFaddend period around 2-4 years \DIFdelbegin \DIFdel{, confirming the }\DIFdelend \DIFaddbegin \DIFadd{throughout the simulation, corresponding to the period of the QBO which supports the }\DIFaddend presence of a Holton-Tan relationship between the QBO and the polar vortex in the model. The \DIFdelbegin \DIFdel{signal is intermittent throughout the simulation and the global spectrum (}\DIFdelend \DIFaddbegin \DIFadd{so-called 'global power spectrum' (i.e. }\DIFaddend the time average of the wavelet spectrum\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend shown on the right of the figure \DIFdelbegin \DIFdel{) }\DIFdelend shows that the signal is on the boundary of \DIFdelbegin \DIFdel{assigned }\DIFdelend \DIFaddbegin \DIFadd{the 95\% }\DIFaddend statistical significance. Other signals at periods near 20-30 years are similarly intermittent and manifest as a peak in the time-averaged spectrum that is also near the \DIFdelbegin \DIFdel{assigned }\DIFdelend \DIFaddbegin \DIFadd{95\% }\DIFaddend significance boundary. The most persistent feature of the series appears at periods between $\sim$60-90 years in the interval between 400-800 yrs. This feature shows statistical significance (based on comparisons between power in the spectrum and that of an AR1 process with the same autocorrelation structure as the series being analysed) for around 350 years of the 1000-yr simulation but does not cross the significance threshold for the time-averaged spectra. There is a possible limitation of this wavelet methodology due to the discrete nature of the time series being analysed (time points take values 0/1/2). The Morlet wavelet is a continuous function and, as a result, convolution with a highly discretised series may alias features on the resulting wavelet spectra. This limitation must be considered when drawing conclusions from the wavelet spectra and is discussed further below.

\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.8\linewidth]{figures/SSW_wavelet.png}
%DIFDELCMD < %%%
...
\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.8\linewidth]{figures/SSW_wavelet_5_yr_fig3.png}
%DIFDELCMD < %%%
...
The most obvious potential driver of such long timescale variability is the ocean due to its high degree of thermal inertia. Previous work has identified coupling between tropical SSTs and the polar vortex, such as the relationship to ENSO conditions (see section 1). \DIFdelbegin \DIFdel{Figure 8A }\DIFdelend \DIFaddbegin \DIFadd{The model exhibits an expected connection between ENSO and the vortex on interannual timescales indicated by the regression analysis results (table 1) and ZMZW composites for El Ni\~{n}o and La Ni\~{n}a winters (supp figure A2). Figure 8a }\DIFaddend shows the wavelet power spectrum for the 5 year smoothed Sep-Nov ENSO 3.4 index as well as the cross power spectrum with $SSW_{5yr}$. We use the early NH winter ENSO index to capture the lagged response of the vortex to this mode of variability. The ENSO index is slowly varying so will likely remain in the same state between early and mid-winter. We also smooth the ENSO index for the purposes of calculating the cross spectrum with $SSW_{5yr}$. (The spectrum of the un-smoothed ENSO 3.4 index is provided in supp figure \DIFdelbegin \DIFdel{A1 }\DIFdelend \DIFaddbegin \DIFadd{A3 }\DIFaddend and shows significant power in the expected period range of 4-7 years \citep{Santoso2017}). The smoothed ENSO 3.4 index shows intermittent power at periods around 16 years which appears significant in the global spectrum. It also exhibits a small signal coincident with the 90 year variability in $SSW_{5yr}$, however this feature only persists for around 100 years of the simulation. Cross spectra between the two series (figure \DIFdelbegin \DIFdel{8B}\DIFdelend \DIFaddbegin \DIFadd{8b}\DIFaddend ) reveals that the coincidence in signals at the 90 year period, while significant under our test, is marginally prominent but only covers a small proportion of significant signals in $SSW_{5yr}$. This suggests there may be some contribution from ENSO to the observed SSW variability but it is only marginally significant and on its own it cannot explain the signal in $SSW_{5yr}$ that persists for 450 years. \DIFaddbegin \DIFadd{The source of this ENSO signal at 90 year periods is unclear, although the PDO spectrum shares some of the same characteristics on the 90 year timescale (supp figure A4) which is consistent with results of \mbox{%DIFAUXCMD
\cite{Newman2016} }\hspace{0pt}%DIFAUXCMD
which proposes the PDO as a low pass filtered version of ENSO. 
}\DIFaddend 

\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/ENSO_wavelet_combined.png}
%DIFDELCMD < %%%
...
The strength of the Aleutian Low (AL) has also been used as an indicator of tropospheric wave forcing and its influence on the polar vortex \citep{Woo2015}. A similar wavelet and cross-spectrum analysis was therefore performed using an index based on the strength of the modelled NH winter (Dec-Mar) AL (see section 2 for details). The wavelet power spectrum for the 5-year smoothed AL index (figure \DIFdelbegin \DIFdel{9A}\DIFdelend \DIFaddbegin \DIFadd{9a}\DIFaddend ) exhibits elements of periodic signals with maximum power corresponding to a period of around 55 years (between 40-60 years) but with fairly minimal overlap with the regions enclosed by the 95\% confidence level in the corresponding SSW wavelet analysis (green contours). \DIFaddbegin \DIFadd{AL indices derived from different winter months give similar spectral patterns (not shown).  }\DIFaddend The cross spectrum analysis between the AL and $SSW_5yr$ (figure \DIFdelbegin \DIFdel{9B}\DIFdelend \DIFaddbegin \DIFadd{9b}\DIFaddend ) highlights this relatively small region of overlap in the interval between years 400-500. However, the \DIFdelbegin \DIFdel{indicated phase relationship}\DIFdelend \DIFaddbegin \DIFadd{phase relationship, indicated by the arrows }\DIFaddend in that region of overlap is difficult to interpret. The proposed physical mechanism of coupling between the AL and the vortex \citep{Woo2015} involves an association between a deeper AL (i.e. lower pressure \DIFaddbegin \DIFadd{and hence a negative anomaly}\DIFaddend ) with increased frequency of SSWs. This negative correlation would give rise to arrows pointing to the left if the relationship was present. In contrast, the upward arrows \DIFdelbegin \DIFdel{figure 9B }\DIFdelend \DIFaddbegin \DIFadd{in figure 9b }\DIFaddend indicate a $\frac{\pi}{2}$ phase difference between the indices on these 60 year timescales, suggesting that peaks in $SSW_{5yr}$ variations are associated with maximum rates of change of the AL index at the same periods. \DIFdelbegin \DIFdel{Given the }\DIFdelend \DIFaddbegin \DIFadd{As with ENSO3.4, the spectra of the AL shares some features with that of the PDO (supp figure A4). This is consistent with studies into these modes of variability which find significant correlation of the PDO and AL \mbox{%DIFAUXCMD
\citep{Mantua_1997, Rodionov2005} }\hspace{0pt}%DIFAUXCMD
as well as studies that examine influence of PDO on vortex strength through a pathway involving the AL and ENSO \mbox{%DIFAUXCMD
\citep{rao2019}}\hspace{0pt}%DIFAUXCMD
. Despite this possible pathway, the }\DIFaddend relatively short time interval of overlap between the AL and $SSW_{5yr}$ signals at the 60-yr period, the absence of any significant signal around the 90-yr period, together with the inconsistent phase relationships\DIFdelbegin \DIFdel{noted above, we conclude that the long-term variability in $SSW_{5yr}$ }\DIFdelend \DIFaddbegin \DIFadd{, points to a conclusion that AL forcing }\DIFaddend is unlikely to be \DIFdelbegin \DIFdel{associated with AL forcing in this simulation}\DIFdelend \DIFaddbegin \DIFadd{the primary driver of long-term variability in $SSW_{5yr}$}\DIFaddend . Indeed, examination of the cross-spectrum between the un-smoothed AL and \DIFdelbegin \DIFdel{SSWs per season }\DIFdelend \DIFaddbegin \DIFadd{SSW }\DIFaddend indices (figure \DIFdelbegin \DIFdel{A4) shows no }\DIFdelend \DIFaddbegin \DIFadd{A7) shows little }\DIFaddend indication of a coherent relationship between the two indices at any timescale\DIFdelbegin \DIFdel{, and the Pearson correlation between the two indices is only 0.21}\DIFdelend \DIFaddbegin \DIFadd{. Finally, while the regression results of the un-smoothed indices give a significant coefficient for the AL (table 1), its magnitude is small compared to that of ENSO3.4 and the deep QBO, the uncertainty on the coefficient is large and the associated p value is close to the 95\% significance boundary}\DIFaddend .

%---------------------------------------------------------------

\begin{center}
\begin{figure}[h!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/AL_wavelet_combined.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \linewidth]{new_changed_figures/AL_wavelet_combined_new_levels.png}
\DIFaddendFL \caption{As figure 8 for the Dec-Mar Aleutian Low index smoothed with a 5 year window. \textbf{a}: AL time series and associated wavelet power spectrum. \textbf{b}: Cross power spectrum between AL and $SSW_{5yr}$.}
\end{figure}
\end{center}
...
\begin{center}
\begin{figure}[h!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \textwidth]{figures/deep_QBO_wavelet_combined.png}
%DIFDELCMD < %%%
...
\begin{tabular}{|p{3cm}||p{3cm}|p{3cm}|}
 ...
 \DIFaddFL{Regression Variable}& \DIFaddFL{Coefficient}& \DIFaddFL{p value}\\
 \hline
 \DIFaddFL{ENSO3.4  }& \DIFaddFL{-0.0127$\pm$0.032}& \DIFaddFL{0.688}\\
 \DIFaddFL{AL  }&   \DIFaddFL{-0.072$\pm$0.021  }& \DIFaddFL{0.046}\\
 \DIFaddFL{deep QBO amp }&\DIFaddFL{-0.124$\pm$0.031}&\DIFaddFL{0.0001}\\
 \hline
\end{tabular}
\DIFaddendFL \begin{center}
\DIFdelbeginFL %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \includegraphics[width = 0.8\linewidth]{figures/cross_power_SSWs_low_rates_vs_QBO_amplitude_modulation_5yr_mean.png}
%DIFDELCMD < %%%
\DIFdelendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Top}}%DIFAUXCMD
\DIFdelFL{: $SSW_{5yr}$ time series with variability in high }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Summary of results from multi-linear regression analysis of }\DIFaddendFL SSW\DIFdelbeginFL \DIFdelFL{rate intervals removed by setting all rates above the climatological mean (0.54 events per season) to the mean (blue) and Sep-Nov deep QBO Hilbert Amplitude index smoothed with a 5 year window (red)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$_{5yr}$}\DIFaddendFL .\DIFdelbeginFL \textbf{\DIFdelFL{Bottom}}%DIFAUXCMD
\DIFdelFL{: Cross wavelet power spectrum between the two time series.}\DIFdelendFL } 
...
\caption{\textbf{\DIFaddFL{Top}}\DIFaddFL{: $SSW_{5yr}$ time series with variability in high SSW rate intervals removed by setting all rates above the climatological mean (0.54 events per season) to the mean (blue) and Sep-Nov deep QBO Hilbert Amplitude index smoothed with a 5 year window (red). }\textbf{\DIFaddFL{Bottom}}\DIFaddFL{: Cross wavelet power spectrum between the two time series.}}
\label{fig1}
\end{figure}
\end{center}

\DIFaddend An obvious question is whether this sensitivity to deep QBO westerlies that we find in the model is also present in the real atmosphere. Examination of the ERA-Interim dataset shows limited support. Some winters in the 1990s are characterised by anomalously westerly Sep-Nov equatorial winds which are vertically coherent between the 15 and 30\,hPa levels. However this effect is intermittent and does not span the whole interval of the 1990s during which SSWs were markedly absent in the observational record \DIFaddbegin \DIFadd{(not shown)}\DIFaddend . On the other hand, a mini hiatus \DIFaddbegin \DIFadd{that was present }\DIFaddend in the mid 2010s is associated with 3 years of deep westerly anomaly in the QBO. Overall, it is clear that the \DIFdelbegin \DIFdel{signal}\DIFdelend \DIFaddbegin \DIFadd{relationship}\DIFaddend , if present \DIFaddbegin \DIFadd{in the real atmosphere}\DIFaddend , is likely obscured by other factors including greenhouse gas increases and volcanic eruptions and the observational dataset is too short to provide useful validation for these long timescale variations.  

 
\section{Summary and Discussion}

In this study, we have examined variability in the appearance of hiatus and intervals of consecutive SSWs in a 1000-yr pre-industrial control simulation of the UK Earth System Model. While there is much observational evidence for an impact of SSWs on the underlying tropospheric weather and climate\DIFdelbegin \DIFdel{the observational record is too short to draw robust conclusions about }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend their multi-decadal variability \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the associated forcing mechanisms \DIFaddbegin \DIFadd{are not well understood due to the short observational record}\DIFaddend . Analysis of long climate model simulations is currently the only available tool for understanding this variability. 
...
A second possible source of long-term variability involving variations in the QBO was also investigated. A range of QBO indices were considered, including the standard approach of using equatorial winds at a specified pressure level e.g. 50 hPa and also a 'deep QBO'  index which takes the average QBO wind over 15-30 hPa, designed to capture the degree of vertical coherence in the QBO winds (following \cite{Gray2018} and \cite{Andrews2019}). A straightforward wavelet analysis of these QBO indices reveals no power at periodicity longer than 2-4 years. However, while there is evidently no long-term variability in the frequency of the QBO, visual examination of the QBO time series clearly shows the presence of  long-term variability in the QBO amplitudes. A measure of this amplitude modulation was extracted by taking the Hilbert Transform of the QBO index. Wavelet analysis of the amplitude variations from the Hilbert Transform of the QBO indices showed long-term periodicity matching that seen in the $SSW_{5yr}$ wavelet analysis. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In particular, the }\DIFaddend deep QBO index \DIFdelbegin \DIFdel{showed greatest overlap with the }\DIFdelend \DIFaddbegin \DIFadd{exhibited significant signals coincident with those in }\DIFaddend $SSW_{5yr}$ \DIFdelbegin \DIFdel{power spectrum, particularly at }\DIFdelend \DIFaddbegin \DIFadd{corresponding to }\DIFaddend periodicities of around 90 years. This overlap accounted for nearly all 450 years of SSW variability present on the 90 year timescale. \DIFaddbegin \DIFadd{Regression analysis of 5 year smoothed and filtered indices confirmed the contribution of the deep QBO amplitude to variability in SSWs on timescales greater than 60 years. 
}\DIFaddend 

Our analysis has therefore revealed an unexpected relationship between the strength and vertical coherence of the QBO and long-term variability in the frequency of SSWs. The relationship was found to be particularly sensitive to the QBO westerly phase. Extended periods of deep westerly QBO phases were associated with hiatus periods with few or no SSWs, consistent with the Holton Tan relationship. \DIFaddbegin \DIFadd{While this result appears compelling, it should be noted that the model showed some biases in its QBO associated with the period and descent rate of shear zones. The extended period of close to 3 years could introduce an element of phase locking between the QBO and seasonal cycle causing winter months to occur preferentially in one QBO phase over the other (evident from figure 5, legends). This may influence QBO-vortex coupling. However, these biases are common in modern GCMs \mbox{%DIFAUXCMD
\citep{Bushell2020} }\hspace{0pt}%DIFAUXCMD
and the pi-control remains the most effective tool for studying multi-decadal variability in the stratosphere.
Recent work has also shown a large degree of inter-model variability exists in representations of the QBO as well as SSWs \mbox{%DIFAUXCMD
\citep{Bushell2020,Ayarz2020} }\hspace{0pt}%DIFAUXCMD
so it is possible the result from this study is model dependant. Additional analysis of long simulations from different models is required to verify these results. 
}\DIFaddend 

Combining the results of all these analyses, our overall  conclusion is that multi-decadal variability in SSW frequency in the model is primarily accounted for by long term variability in QBO-SSW coupling, particularly at periodicities of around 90 years and, to a lesser extent, by variability in the \DIFdelbegin \DIFdel{depth }\DIFdelend \DIFaddbegin \DIFadd{intensity }\DIFaddend of the Aleutian Low at periodicities around 60 years, although coherence with the AL signals is far less persistent than with the QBO. Given the observed impact of SSWs on the underlying tropospheric weather and climate, improved understanding of the source and mechanisms of long-term variability in QBO-SSW interactions is likely to help improve future seasonal weather forecasts and decadal-scale climate predictions. The precise nature of QBO-SSW interaction mechanisms are still not fully understood \citep{Anstey2014}. While the importance of wave-mean flow interactions is widely recognised, further studies are required to explore the relevance and usefulness of the deep QBO index highlighted in this study, that identifies a vertically coherent QBO phase. It appears to be especially relevant to long-term QBO-SSW interactions during the QBO-W phase. 
...
\caption{\textbf{\DIFaddFL{Top left}}\DIFaddFL{: SSW events per Nov-Mar season in UKESM smoothed using a 5 year running mean. }\textbf{\DIFaddFL{Bottom left}}\DIFaddFL{: Wavelet power spectrum of time series in top left. Hatching represents area outside the cone of influence in which edge effects are significant and power should not be considered. Yellow contours represent 95\% confidence level assuming mean background AR1 red noise. }\textbf{\DIFaddFL{Top Right}}\DIFaddFL{: Morlet wavelet used for the wavelet transform in the time domain. }\textbf{\DIFaddFL{Bottom right:}} \DIFaddFL{Global power spectrum, the wavelet power averaged over the whole simulation, and global 95\% confidence spectrum.}}
\label{fig3}
\end{figure}
\end{center}
\DIFaddend 
...
\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/ENSO3.4_wavelet_UKESM.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \linewidth]{new_changed_figures/ZMZW_ENSO_phase_1K.png}
\caption{\DIFaddFL{Dec-Mar ZMZW anomaly composites for positive (El Ni\~{n}o) and negative (La Ni\~{n}a) phases of the ENSO3.4 index evaluated in Sep-Nov. The phase of ENSO3.4 is defined as an SST anomaly of greater magnitude than 1K. Coloured shading indicates anomalies significant above the 95\% confidence level under a 2 tail student’s t-test.}}
\label{fig3}
\end{figure}
\end{center}
...
\DIFaddendFL \caption{\textbf{Top left}: Sep-Nov ENSO3.4 index from the UKESM pi-control simulation. \textbf{Bottom left}: Wavelet power spectrum of time series in top left. Hatching represents area outside the cone of influence in which edge effects are significant and power should not be considered. Yellow contours represent the 95\% confidence level assuming mean background AR1 red noise. \textbf{Top Right}: Morlet wavelet used for the wavelet transform in the time domain. \textbf{Bottom right:} Global power spectrum, the wavelet power averaged over the whole simulation, and global 95\% confidence spectrum.}
\label{fig3}
\end{figure}
\end{center}
...
\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/SSTs_tropical_wavelet.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \linewidth]{new_changed_figures/PDO_wavelet_combined_new_levels.png}
\caption{\textbf{\DIFaddFL{(A, top)}}\DIFaddFL{: Sep-Nov PDO time series, }\textbf{\DIFaddFL{A, bottom left}}\DIFaddFL{: Wavelet power spectrum (shaded contours represent wavelet power and yellow contours the 95\% significance level compared to an AR1 process), }\textbf{\DIFaddFL{A, bottom right}}\DIFaddFL{: global wavelet power spectrum (blue) and 95\% confidence level (dashed red). }\textbf{\DIFaddFL{B}}\DIFaddFL{: Cross spectra between $SSW_5yr$ and the PDO index. }\textbf{\DIFaddFL{B, top}}\DIFaddFL{: PDO and $SSW_5yr$ time series. }\textbf{\DIFaddFL{B, bottom}}\DIFaddFL{: Cross power spectrum. Shading indicates cross power, yellow contours the 95\% confidence interval and arrows the relative phase angle between signals in the time series. Green contours on both spectra represent the 95\% confidence intervals for the wavelet power spectrum of $SSW_{5yr}$.}}
\end{figure}
\end{center}
...
\begin{center}
\begin{figure}[h!]
\noindent\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/SSW_5yr_vs_trop_SSTs_crosspower.png}
%DIFDELCMD < %%%
...
\begin{center}
\begin{figure}[h!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/cross_power_SSWs_vs_AL_unsmoothed.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = \linewidth]{new_changed_figures/cross_power_SSWs_vs_AL_new_levels.png}
\DIFaddendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Top left}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Top}}\DIFaddendFL : Dec-Mar Aleutian Low index \DIFdelbeginFL \DIFdelFL{from the UKESM pi-control simulation. }\textbf{\DIFdelFL{Bottom left}}%DIFAUXCMD
\DIFdelFL{: Wavelet power spectrum of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(red) and SSWs per NH winter (blue) }\DIFaddendFL time series\DIFdelbeginFL \DIFdelFL{in top left}\DIFdelendFL . \DIFdelbeginFL \DIFdelFL{Hatching represents area outside the cone of influence in which edge effects are significant and power should not be considered. Yellow contours represent 95\% confidence level assuming mean background AR1 red noise. }\textbf{\DIFdelFL{Top Right}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Bottom}}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{Morlet }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Cross }\DIFaddendFL wavelet \DIFdelbeginFL \DIFdelFL{used for the wavelet transform in the time domain. }\textbf{\DIFdelFL{Bottom right:}} %DIFAUXCMD
\DIFdelFL{Global }\DIFdelendFL power spectrum \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{between }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{wavelet power averaged over the whole simulation, and global 95\% confidence spectrum}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{two time series}\DIFaddendFL .}
\end{figure}
\end{center}

\begin{center}
\begin{figure}[h!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.8\linewidth]{figures/cross_power_ENSO_vs_deep_QBO_amp_5yr_mean.png}
%DIFDELCMD < %%%
...
\begin{center}
\begin{figure}[h!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = \linewidth]{figures/QBO_amp_vs_trop_SSTs_crosspower.png}
%DIFDELCMD < %%%
